...
import {
  assignmentGenderGroup,
  assignmentStudentName,
  sortAssignmentStudents,
  type AssignmentGenderGroup,
} from "./studentSorting";
      ...
      <div className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-none border-2 border-black bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
        <div className="flex items-start justify-between border-b-2 border-black bg-primary p-4">
          ...
          <button
            ...
            onClick={onClose}
            className="border-2 border-black bg-white p-1 hover:bg-gray-100"
          >
            <X className="size-5" />
          </button>
        </div>
        <div className="grid gap-4 overflow-y-auto p-4">
          <div className="grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
            ...
            <Stat
              label="Female"
              value={groupStudents(sorted, "female").length}
            />
          </div>
          <div className="grid gap-4 md:grid-cols-2">
            {(["male", "female"] as AssignmentGenderGroup[]).map((group) => (
              <DetailsGroup
                key={group}
                group={group}
                students={groupStudents(sorted, group)}
              />
            ))}
          </div>
        </div>
...
function groupStudents(
  students: ClassAssignmentStudent[],
  group: AssignmentGenderGroup,
) {
  return students.filter(
    (student) => assignmentGenderGroup(student.gender) === group,
  );
}
...
function DetailsGroup({
  ...
  return (
    <section>
      <h4 className="border-b-2 border-black pb-1 text-sm font-bold">
        {labels[group]} ({students.length})
      </h4>
      {!students.length && (
        <p className="py-2 text-xs text-black/50">No students</p>
      )}
      {students.map((student, index) => (
        <div
          key={student.student_id}
          className="grid grid-cols-[28px_1fr] border-b border-black/15 py-2 text-xs last:border-0"
        >

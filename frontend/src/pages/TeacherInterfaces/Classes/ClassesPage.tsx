...
function DashboardCard({
  ...
  return (
    ...
    >
      <h2 className="truncate text-2xl font-bold leading-tight text-gray-950">{title}</h2>
      {subtitle && <p className="text-sm font-semibold text-gray-800">{subtitle}</p>}
...
const ClassesPage = () => {
  ...
  useEffect(() => {
    const loadTeacherClasses = async () => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : "Unable to load teacher classes.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const subjects = useMemo(() => {
    ...
    loads.forEach((load) => {
      bySubject.set(load.subject_id, [...(bySubject.get(load.subject_id) || []), load]);
    });
                ...
                {subjects.map((subject) => (
                  <DashboardCard
                    ...
                    onClick={() => {
                      ...
                      if (firstClass) {
                        navigate(`/teacher/classes/${firstClass.class_id}/subjects/${subject.subjectId}`);
                      }
                    }}
                  />
                ))}
                ...
                {classes.map((item) => {
                  ...
                  return (
                    <DashboardCard
                      ...
                      onClick={() => {
                        if (primarySubject) {
                          navigate(`/teacher/classes/${item.classId}/subjects/${primarySubject.subject_id}`);
                        }
                      }}
                    />
                  );

...
export default function AIQuizGeneratorModal({
  ...
  const handleGenerate = async () => {
    if (sourceMode === "specific" && readingClassworks.length > 0 && selectedReadingIds.length === 0) {
      setError("Please select at least one reading classwork in 'By Specific Readings' mode.");
      return;
    }
    if (totalItemCount <= 0) {
    ...
    try {
      ...
      let derivedLessonTitles: string[] = [];
      ...
      if (sourceMode === "specific") {
        ...
        for (const r of selectedItems) {
          if (r.lesson_id) idSet.add(r.lesson_id);
          if (r.lesson_title) titleSet.add(r.lesson_title);
        }

        derivedLessonIds = Array.from(idSet);
        derivedLessonTitles = Array.from(titleSet);
      } else {
        derivedLessonIds = selectedLessonIds;
        derivedLessonTitles = lessons
          .filter((l) => selectedLessonIds.includes(l.lesson_id))
          .map((l) => l.title);
      }

      // Hand off directly to Question Review in Quiz Builder
      onGenerated(
        drafts,
        data.warnings ?? [],
        currentSubjectId,
    ...
    >
      ...
      >
        ...
        <section className="flex flex-col gap-4 p-5">
          ...
          <div className="space-y-2">
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            className="rounded border-2 border-black accent-black w-4 h-4 cursor-pointer"
                          />
            ...
            {sourceMode === "specific" && (
              <div className="space-y-1.5">
                ...
                <p className="text-[11px] text-muted-foreground">
                  {selectedReadingIds.length} reading classwork(s) selected across lessons.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold uppercase tracking-wider text-black">
              Specific Coverage / Instructions (Optional)
            </label>
            <textarea
              value={additionalCoverage}
              onChange={(e) => setAdditionalCoverage(e.target.value)}
              placeholder="e.g., Focus on Chapter 3 photosynthesis; include questions on ATP cycle; emphasize terminology..."
              rows={2}
              disabled={isGenerating}
              className="w-full border-2 border-black p-2.5 text-xs font-medium bg-white shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] resize-none focus:outline-none placeholder:text-muted-foreground"
            />
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-xs font-bold uppercase tracking-wider text-black">
                Test Parts & Difficulties
              </label>
              <span className="text-[11px] font-extrabold text-black">
                Total: {totalItemCount} items ({totalPointsCount} pts)
              </span>
            </div>
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            const clamped = isNaN(parsed) ? 1 : Math.max(1, Math.min(50, parsed));
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            const clamped = isNaN(parsed) ? 1 : Math.max(0.5, Math.min(100, parsed));
                            e.target.value = String(clamped);
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                const clamped = isNaN(parsed)
                                  ? 0
                                  : Math.max(0, Math.min(part.count, parsed));
                                e.target.value = String(clamped);
                                updateBreakdown(part.id, diff, clamped);
                              }}
                              className="w-12 h-7 text-center text-xs font-bold border-2 border-black rounded focus:outline-none focus:ring-1 focus:ring-black"
                            />
                          </div>
                        ))}
                        {bError && (
